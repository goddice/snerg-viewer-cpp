...
int main(int argc, char** argv)
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "SNeRG Viwer", NULL, NULL);
    if (window == NULL)
    {
    ...
    float cy = 1.0f;
    float cz = -4.0f;
    if ((bool)scene.gSceneParams["ndc"]) {
        cz = -0.25f;
    }

    Camera3D cam3d(glm::vec3(cx, cy, cz));
    cam3d.LookAt(glm::vec3(0.0f, 0.0f, 0.0f));
    TrackballControls* tball = &TrackballControls::GetInstance(&cam3d, glm::vec4(0.0f, 0.0f, (float)SCR_WIDTH, (float)SCR_HEIGHT));
    tball->Init(window);
