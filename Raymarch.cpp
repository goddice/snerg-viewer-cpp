...
bool Raymarch::initScene() {
    ...
    shader.setInt("ndc", 0);
    shader.setFloat("nearPlane", 0.33);
...
bool Raymarch::loadScene(const std::string& dirUrl, int width, int height) {
    ...
    glBindTexture(GL_TEXTURE_3D, rgbVolumeTexture);
    glTexStorage3D(GL_TEXTURE_3D, 1, GL_RGB8, atlas_width, atlas_height, atlas_depth);
    ...
    glBindTexture(GL_TEXTURE_3D, alphaVolumeTexture);
    glTexStorage3D(GL_TEXTURE_3D, 8, GL_R8, atlas_width, atlas_height, atlas_depth);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    for (int i = 0; i < gNumTextures; i++) {
        ...
        for (int z = 0; z < slice_depth; ++z) {
            for (int y = 0; y < atlas_height; ++y) {
                glTexSubImage3D(
                    GL_TEXTURE_3D, 0, 0, y, z + i * slice_depth,
                    atlas_width, 1, 1, GL_RGB, GL_UNSIGNED_BYTE,
        ...
        for (int z = 0; z < slice_depth; ++z) {
            for (int y = 0; y < atlas_height; ++y) {
                glTexSubImage3D(
                    GL_TEXTURE_3D, 0, 0, y, z + i * slice_depth,
                    atlas_width, 1, 1, GL_RED, GL_UNSIGNED_BYTE,
    ...
    if (!gSceneParams["diffuse"]) {
        ...
        glBindTexture(GL_TEXTURE_3D, featureVolumeTexture);
        glTexStorage3D(GL_TEXTURE_3D, 1, GL_RGBA8, atlas_width, atlas_height, atlas_depth);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        ...
        for (int i = 0; i < gNumTextures; i++) {
            ...
            for (int z = 0; z < slice_depth; ++z) {
                for (int y = 0; y < atlas_height; ++y) {
                    glTexSubImage3D(
                        GL_TEXTURE_3D, 0, 0, y, z + i * slice_depth,
                        atlas_width, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE,
    ...
    unsigned long long indexImageWidth = std::ceil(1.0 * grid_width / block_size);
    unsigned long long indexImageHeight = std::ceil(1.0 * grid_height / block_size);
    unsigned long long indexImageDepth = std::ceil(1.0 * grid_depth / block_size);
    std::cout << "Index Image with w=" << indexImageWidth << ", h=" << indexImageHeight << ", d=" << indexImageDepth << "\n";
    glTexStorage3D(GL_TEXTURE_3D, 1, GL_RGBA8, indexImageWidth, indexImageHeight, indexImageDepth);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    glTexSubImage3D(
        GL_TEXTURE_3D, 0,
        0, 0, 0,
        indexImageWidth, indexImageHeight, indexImageDepth, GL_RGBA, GL_UNSIGNED_BYTE,
    ...
    createPlaneBufferGeometry(width, height, -100);
...
GLuint Raymarch::createNetworkWeightTexture(nlohmann::json network_weights) {
    int width = network_weights.size();
    int height = network_weights[0].size();
    ...
    return createFloatTextureFromData(width, height, weightsData.data());
}
...
std::string Raymarch::createViewDependenceFunctions() {
    ...
    int width = network_weights["0_bias"].size();
    ...
    int channelsZero = network_weights["0_weights"].size();
    int channelsOne = network_weights["0_bias"].size();
    int channelsTwo = network_weights["1_bias"].size();
    int channelsThree = network_weights["2_bias"].size();
    int posEncScales = 4;
